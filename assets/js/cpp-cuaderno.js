                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response && response.success && response.data && typeof response.data.html_cuaderno !== 'undefined') {
                            ...
                            // Restaurar la posición del scroll si existe, con un timeout para asegurar el renderizado
                            if (self.savedScroll) {
                                const $scrollContainer = $('.cpp-cuaderno-tabla-wrapper');
                                if ($scrollContainer.length) {
                                    setTimeout(() => {
                                        $scrollContainer.scrollTop(self.savedScroll.top);
                                        $scrollContainer.scrollLeft(self.savedScroll.left);
                                        self.savedScroll = null; // Limpiar después de usar
                                    }, 0);
                                }
                            }
                        } else {
