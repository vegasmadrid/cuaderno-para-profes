...
(function($) {
    ...
    cpp.alumnos = {
        ...
        bindEvents: function() {
            ...
            // Listener para mostrar el botón de guardar al detectar cambios
            $document.on('input change', '#cpp-unified-alumno-form', function() {
                $('#cpp-unified-save-btn').show();
            });
        ...
        renderAlumnoFicha: function(data) {
            ...
            const unifiedFormHtml = `
                <div class="cpp-alumno-ficha-header">
                    ...
                    <div class="cpp-form-actions">
                        <button type="button" id="cpp-unified-save-btn" class="cpp-btn cpp-btn-primary" data-alumno-id="${alumno.id}" style="display: none;">
            ...
            if (!isNew) {
                ...
                $('#cpp-unified-alumno-form').attr({
                    'data-initial-ids': JSON.stringify(data.clases_del_alumno_ids),
                    'data-ids-with-grades': JSON.stringify(idsWithGrades)
                });
        ...
        handleUnifiedSave: function(e) {
            ...
            const $form = $('#cpp-unified-alumno-form');
            const self = this;

            // Lógica de advertencia (se ejecuta antes de cualquier acción)
            const initialIds = JSON.parse($form.attr('data-initial-ids') || '[]');
            const idsWithGrades = JSON.parse($form.attr('data-ids-with-grades') || '[]');
