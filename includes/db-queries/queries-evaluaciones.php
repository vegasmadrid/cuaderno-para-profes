...
function cpp_obtener_evaluaciones_por_clase($clase_id, $user_id) {
    ...
    $evaluaciones = $wpdb->get_results($wpdb->prepare(
        "SELECT id, clase_id, user_id, nombre_evaluacion, start_date, start_evaluacion_id, calculo_nota, orden, fecha_creacion FROM $tabla_evaluaciones WHERE clase_id = %d AND user_id = %d ORDER BY orden ASC, fecha_creacion ASC",
        $clase_id, $user_id
    ), ARRAY_A);
