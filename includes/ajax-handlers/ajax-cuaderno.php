...
function cpp_ajax_cargar_cuaderno_clase() {
    ...
    $clase_db = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabla_clases WHERE id = %d AND user_id = %d", $clase_id, $user_id));
        ...
        if ($evaluacion_id_solicitada) {
            foreach ($evaluaciones as $eval) {
                if ($eval['id'] == $evaluacion_id_solicitada) {
                    $evaluacion_activa = $eval;
                    break;
                }
            }
        }
        if ($evaluacion_activa === null && isset($evaluaciones[0])) {
            $evaluacion_activa = $evaluaciones[0];
        }
    }

    if ($evaluacion_activa === null) {
        wp_send_json_error(['message' => 'No se encontraron evaluaciones para esta clase.']);
        return;
    }

    $evaluacion_activa_id = $evaluacion_activa['id'];
    $is_final_evaluation = ($evaluacion_activa['tipo'] === 'final');

    $alumnos = cpp_obtener_alumnos_clase($clase_id);
    $base_nota_final_clase = floatval($clase_db->base_nota_final);
    $clase_color_actual = !empty($clase_db->color) ? $clase_db->color : '#2962FF';
    ...
    if ($is_final_evaluation) {
        // --- VISTA PARA LA EVALUACIÓN FINAL ---
        $evaluaciones_normales = $wpdb->get_results($wpdb->prepare(
            "SELECT id, nombre_evaluacion FROM $tabla_evaluaciones WHERE clase_id = %d AND user_id = %d AND tipo = 'normal' ORDER BY orden ASC",
            $clase_id, $user_id
        ), ARRAY_A);
        ?>
        <div class="cpp-cuaderno-tabla-wrapper">
            <table class="cpp-cuaderno-tabla">
                <thead>
                    <tr>
                        <th class="cpp-cuaderno-th-alumno">Alumno/a</th>
                        <?php foreach ($evaluaciones_normales as $eval_normal): ?>
                            <th><?php echo esc_html($eval_normal['nombre_evaluacion']); ?></th>
                        <?php endforeach; ?>
                ...
                <tbody>
                    <?php if (empty($alumnos)): ?>
                        <tr><td colspan="<?php echo count($evaluaciones_normales) + 2; ?>">No hay alumnos en esta clase.</td></tr>
                    <?php else: foreach ($alumnos as $alumno): ?>
                        <tr>
                            <td class="cpp-cuaderno-td-alumno">
                                <div class="cpp-alumno-avatar-cuaderno"><span><?php echo strtoupper(substr(esc_html($alumno['nombre']), 0, 1)); ?></span></div>
                                <span class="cpp-alumno-nombre-cuaderno"><?php echo esc_html($alumno['apellidos'] . ', ' . $alumno['nombre']); ?></span>
                            </td>
                            <?php
                            $suma_notas_evaluaciones = 0;
                            foreach ($evaluaciones_normales as $eval_normal):
                                $nota_eval_100 = cpp_calcular_nota_final_alumno($alumno['id'], $clase_id, $user_id, $eval_normal['id']);
                                $nota_eval_reescalada = ($nota_eval_100 / 100) * $base_nota_final_clase;
                                $suma_notas_evaluaciones += $nota_eval_reescalada;
                                ?>
                                <td style="text-align:center;"><?php echo cpp_formatear_nota_display($nota_eval_reescalada, 2); ?></td>
                            <?php endforeach;
                            $media_final = count($evaluaciones_normales) > 0 ? $suma_notas_evaluaciones / count($evaluaciones_normales) : 0;
                            ?>
                            <td class="cpp-cuaderno-td-final" id="cpp-nota-final-alumno-<?php echo esc_attr($alumno['id']); ?>">
                                <?php echo cpp_formatear_nota_display($media_final, 2); ?>
                            </td>
                        </tr>
        ...
        // --- VISTA PARA EVALUACIONES NORMALES (CÓDIGO ORIGINAL) ---
        ...
        if (!empty($alumnos)) {
            foreach ($alumnos as $alumno) {
                ...
                $notas_finales_alumnos[$alumno['id']] = ($nota_0_100 / 100) * $base_nota_final_clase;
            }
        }
        ?>
        <div class="cpp-cuaderno-tabla-wrapper">
            <table class="cpp-cuaderno-tabla">
                <thead>
                    <tr>
                        <th class="cpp-cuaderno-th-alumno">
                            <div class="cpp-a1-controls-container"><div class="cpp-a1-icons-row">
                        ...
                        <?php else: foreach ($actividades_raw as $actividad):
                                ...
                                if ($evaluacion_activa['calculo_nota'] === 'ponderada' && !empty($actividad['categoria_color']) && !in_array($actividad['nombre_categoria'], ['General', 'Sin categoría'])) {
                            ...
                            ?>
                                <th class="cpp-cuaderno-th-actividad" style="background-color:<?php echo esc_attr($header_bg_color); ?>; color: <?php echo esc_attr($contrasting_text_color); ?>;">
                                    <div class="cpp-editable-activity-name" data-actividad-id="<?php echo esc_attr($actividad['id']); ?>" title="Editar Actividad: <?php echo esc_attr($actividad['nombre_actividad']); ?>">
                                    ...
                                    <?php if ($evaluacion_activa['calculo_nota'] === 'ponderada'): ?>
                ...
                <tbody>
                    <?php if (empty($alumnos)): ?> <tr><td colspan="<?php echo count($actividades_raw) > 0 ? count($actividades_raw) + 2 : 3; ?>">No hay alumnos.</td></tr>
                    <?php else: foreach ($alumnos as $index => $alumno):
                            $nota_final_display = isset($notas_finales_alumnos[$alumno['id']]) ? cpp_formatear_nota_display($notas_finales_alumnos[$alumno['id']], 2) : '-'; ?>
                            <tr data-alumno-id="<?php echo esc_attr($alumno['id']); ?>"><td class="cpp-cuaderno-td-alumno"><div class="cpp-alumno-avatar-cuaderno"><span><?php echo strtoupper(substr(esc_html($alumno['nombre']),0,1));?></span></div><span class="cpp-alumno-nombre-cuaderno"><?php echo esc_html($alumno['apellidos'] . ', ' . $alumno['nombre']); ?></span></td><?php if (empty($actividades_raw)): ?><td class="cpp-cuaderno-td-no-actividades"></td>
                                <?php else: foreach ($actividades_raw as $actividad):
                                        $nota_alumno_actividad_raw = isset($calificaciones_raw[$alumno['id']][$actividad['id']]) ? $calificaciones_raw[$alumno['id']][$actividad['id']] : ''; ?>
                                        <td class="cpp-cuaderno-td-nota" data-actividad-id="<?php echo esc_attr($actividad['id']); ?>"><input type="text" class="cpp-input-nota" value="<?php echo esc_attr(cpp_formatear_nota_display($nota_alumno_actividad_raw)); ?>" data-alumno-id="<?php echo esc_attr($alumno['id']); ?>" data-actividad-id="<?php echo esc_attr($actividad['id']); ?>" data-nota-maxima="<?php echo esc_attr($actividad['nota_maxima']); ?>" placeholder="-"><span class="cpp-nota-validation-message cpp-error-message" style="display:none;"></span></td>
                                <?php endforeach; endif; ?><td class="cpp-cuaderno-td-final" id="cpp-nota-final-alumno-<?php echo esc_attr($alumno['id']); ?>"><?php echo esc_html($nota_final_display); ?></td></tr>
                        <?php endforeach; endif; ?>
                </tbody>
            </table>
        </div>
    ...
    wp_send_json_success([
        ...
        'calculo_nota' => $evaluacion_activa['calculo_nota'],
        'base_nota_final' => $base_nota_final_clase,
        'nota_minima' => floatval($clase_db->nota_minima)
    ]);
}
